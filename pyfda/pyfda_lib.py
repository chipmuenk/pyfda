...
def fil_save(fil_dict, arg, format_in, sender, convert = True):
    ...
    print("fil_save: shape(arg) = ", np.ndim(arg), np.shape(arg), len(arg))
...
#            a[-1] = 1.
    ...
    if convert:    
        fil_convert(fil_dict, format_in, sender)
...
#==============================================================================
def fil_convert(fil_dict, format_in, sender):
    """
    Convert between poles / zeros / gain, filter coefficients (polynomes)
    ...
    format_in :  string or set of strings
         ...
         'ba' : [b, a] where b are the polynomial coefficients
