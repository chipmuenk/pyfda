...
def save_file_coe(f):
    ...
    bq = qc.float2frmt(fb.fil[0]['ba'][0]) # Quantize coefficients to integer format
    coe_width = qc.W # quantized word length; Int. + Frac. + Sign bit
    ...
    xil_str += datetime.datetime.now().strftime("%d-%B-%Y %H:%M:%S") + "\n;\n"
