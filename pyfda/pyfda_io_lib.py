...
def generate_header(title):
    ...
    header += "Sampling freq.:\t f_S = {0} {1}\n\n".format(fb.fil[0]["f_S"], unit)
    header += "------------------------------------------------------------------------------------\n"
    return header
...
def export_coe_xilinx(f):
    ...
    exp_str += "; Designed:\t{0}\n".format(datetime.datetime.fromtimestamp(int(fb.fil[0]['timestamp'])).strftime(date_frmt))
    exp_str += "; Saved:\t{0}\n;\n".format(datetime.datetime.now().strftime(date_frmt))
    exp_str += "; Filter order = {0}, type: {1}\n".format(fb.fil[0]["N"], fb.fil[0]['rt'])
    exp_str += "; Params:\t f_S = {0}\n".format(fb.fil[0]["f_S"])
    exp_str += "; #############################################################################\n"
    exp_str += "Radix = {0};\n".format(coe_radix)
...
def export_coe_vhdl_package(f):
    ...
    exp_str += "-- Saved:\t{0}\n-- \n".format(datetime.datetime.now().strftime(date_frmt))
    exp_str += "-- Filter order = {0}, type: {1}\n".format(fb.fil[0]["N"], fb.fil[0]['rt'])
    exp_str += "-- Params:\t f_S = {0} {1}\n".format(fb.fil[0]["f_S"], fb.fil[0]['plt_fUnit'])
    exp_str += "-- ------------------------------------------------------------------------------------\n"

    exp_str += "LIBRARY ieee;\n"
