...
class FilterTreeBuilder(object):
    ...
    def __init__(self, filt_dir, filter_file, comment_char='#', DEBUG=False):
        """
        - Extract the names of all Python files in 'filt_dir'/'filter_file'
...
#        self.filt_dir_file = os.path.abspath(cwd + '/' + filt_dir + "/" + filter_file)
        self.filt_dir_file = os.path.join(cwd, filt_dir, filter_file)
        if DEBUG:
    ...
    def buildFilTree(self):
        ...
        for dm in self.design_methods:           # iterate over found designMethods(dm)
            ...
            for rt in cur_filter.rt:            # iterate over response types
                if rt not in fil_tree:           # is rt key in dict already?
                    fil_tree.update({rt:{}})     # no, create it

                if ft not in fil_tree[rt]:  # is ft key already in dict[rt]?
                    fil_tree[rt].update({ft:{}}) # no, create it
                fil_tree[rt][ft].update({dm:{}}) # append dm to list dict[rt][ft]
                # finally append all the individual 'min' / 'man' info
                # to dm in fil_tree. These are e.g. the params for 'min' and /or
                # 'man' filter order
                fil_tree[rt][ft][dm].update(cur_filter.rt[rt])
                ...
                for minman in cur_filter.com:
                    # add info only when 'man' / 'min' exists in fil_tree
                    if minman in fil_tree[rt][ft][dm]:
                        for i in cur_filter.com[minman]:
                            # Test whether entry exists in fil_tree:
                            if i in fil_tree[rt][ft][dm][minman]:
                                # yes, prepend common data
                                fil_tree[rt][ft][dm][minman][i] =\
                                cur_filter.com[minman][i] + fil_tree[rt][ft][dm][minman][i]
                            else:
                                # no, create new entry
                                fil_tree[rt][ft][dm][minman].update(\
                                                {i:cur_filter.com[minman][i]})

                            if self.DEBUG:
                                print('\n--- FilterFileReader.buildFilterTree ---')
                                print(dm, minman, i)
                                print("fil_tree[minman][i]:",
                                      fil_tree[rt][ft][dm][minman][i])
        ...
        if self.DEBUG: print("fil_tree = ", fil_tree)
...
#    import filterbroker as fb
    ...
    myTreeBuilder = FilterTreeBuilder(subDirectory, filtFileName, comment_char, Debug)

    print("\n===== Start Test ====")
    for name in fb['imports']:
