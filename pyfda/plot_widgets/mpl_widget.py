...
from matplotlib import rcParams, axes, lines
...
# ------------------------------------------------------------------------------
def stems(x, y, ax=None, mkr_fmt=None, **kwargs) -> tuple:
    """
    ...
    mkr_fmt : dict, optional
        The format dictionary for the markers.
    **kwargs : dict
        Additional keyword arguments for the stem plot.

    Returns
    -------
    tuple
        A tuple containing the line handles for the stem plot.
    """
...
def scatter(x, y, ax, label: str = "", mkr_fmt: dict = None) -> lines.Line2D:
    """
    Create a wrapper for matplolibs scatter that can handle 'ms' and 'markersize' keys
...
class MplWidget(QWidget):
    ...
    # ----------------------------------------------------------------------------
    def get_full_extent(self, ax: axes.Axes, pad: float = 0.0) -> Bbox:
        """
        ...
        ----------
        ax : matplotlib.axes.Axes
            The axes for which to get the full extent.
        pad : float, optional
            The padding to add around the extent.

        Returns
        -------
        Bbox
            The full extent of the axes.
        """
        # http://stackoverflow.com/questions/14712665/matplotlib-subplot-background\
        # -axes-face-labels-colour-or-figure-axes-coor
    ...
    # ----------------------------------------------------------------------------
    def show_annotation(self, sel: mplcursors.Selection) -> None:
        """
    ...
    # https://stackoverflow.com/questions/59800059/how-to-use-two-mplcursors-simultaneously\
    #   -for-a-scatter-plot-of-two-sets
...
class MplToolbar(NavigationToolbar):
    ...
    see also
    http://stackoverflow.com/questions/17711099/programmatically-change-matplotlib-toolbar\
        -mode-in-qt4
    http://matplotlib-users.narkive.com/C8XwIXah/need-help-with-darren-dale-qt-example-of\
        -extending-toolbar
    https://sukhbinder.wordpress.com/2013/12/16/simple-pyqt-and-matplotlib-example-with-zoompan\

    Changing the info:
    http://stackoverflow.com/questions/15876011/add-information-to-matplotlib-navigation\
        -toolbar-status-bar
    https://stackoverflow.com/questions/53099295/matplotlib-navigationtoolbar-advanced-figure\
        -options
...
# subclass NavigationToolbar, passing through arguments:
    ...
    def emit(self, dict_sig: dict, sig_name: str = "") -> None:
        ...
        ----------
        dict_sig : dict
            The dictionary containing the signal information to be emitted.
    ...
    # ------------------------------------------------------------------------------
    def save_button_states(self) -> None:
    ...
    # ------------------------------------------------------------------------------
    def cycle_draw_grid(self, cycle: bool = True, axes: axes.Axes = None) -> None:
        """
    ...
    # ------------------------------------------------------------------------------
    def enable_plot(self, state: bool = True) -> None:
        """
        ...
        ----------
        state : bool, optional
            Set the plot enabled state to the passed value when state is not None,
            otherwise toggle the state.

            When plot is disabled, the canvas is cleared and all plot-related buttons are disabled,
            except for the enable button itself.

            When plot is enabled, the buttons are enabled according to their previous state before
            disabling the plot.
