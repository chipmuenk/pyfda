...
from pyfda.filterbroker import fb_set, set_fx
...
class TestSequenceFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.my_q = fix_lib.Fixed(q_dict) # instantiate fixpoint object with settings above
...
#            self.assertTrue(element in self.seq)
    def test_write_q_dict(self):
        ...
        """
        fb_set('qfrmt', 'qfrac')
        fb_set('fx_base', 'hex')  # set to fractional format
        q_dict = {'WI':7, 'WF':3, 'ovfl':'none', 'quant':'fix'}
        self.my_q.set_qdict(q_dict)
        # self.assertEqual(q_dict, self.my_q.q_obj)

        self.assertEqual(7, self.my_q.WI)
        self.assertEqual(3, self.my_q.WF)
    ...
    def test_fix_no_ovfl(self):
        ...
        fb_set('qfrmt', 'qfrac')  # set to fractional format
        fb_set('fx_base', 'dec')
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'dec')  # set to int format
        ...
        yq_list = list(self.my_q.fixp(y_string))
        yq_list_goal = [-9, -8, -4, 0, 4, 7, 8, 8, 9]
        self.assertEqual(yq_list, yq_list_goal)
...
#==============================================================================
    def test_fix_saturation(self):
        ...
        self.my_q.set_qdict(q_dict)
        yq_list = list(self.my_q.fixp(y_list_ovfl))
    ...
    def test_fix_wrap(self):
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'dec')  # set to integer format
        q_dict = {'WI':3, 'WF':0, 'ovfl':'wrap', 'quant':'round'}
        self.my_q.set_qdict(q_dict)
        yq_list = self.my_q.fixp(y_list_ovfl)
    ...
    def test_float2frmt_bin(self):
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'bin')  # set to binary integer format
        q_dict = {'WI':3, 'WF':0, 'ovfl':'sat', 'quant':'round'}
        self.my_q.set_qdict(q_dict)

        # test handling of invalid inputs - scalar inputs
        yq_list = list(map(self.my_q.float2frmt, self.y_list_validate))
        yq_list_goal = ["0000", "0000", "0111", "0111", "0000", "0111", "0111"]
        self.assertEqual(yq_list, yq_list_goal)
        # same in vector format
        yq_list = list(self.my_q.float2frmt(self.y_list_validate))
        ...
        self.assertEqual(yq_list, yq_list_goal)
        # same but vectorized function
        yq_list = list(self.my_q.float2frmt(self.y_list))
        self.assertEqual(yq_list, yq_list_goal)
    ...
    def test_float2frmt_hex(self):
        ...
        yq_list = list(map(self.my_q.float2frmt, y_list))
        self.assertEqual(yq_list, yq_list_goal)
        # same, vectorized
        yq_list = list(self.my_q.float2frmt(y_list))
        self.assertEqual(yq_list, yq_list_goal)

        # Fractional case: Q0.6,
        self.my_q.set_qdict({'WI': 0, 'WF': 6})
        yq_list = list(map(self.my_q.float2frmt, y_list))
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'hex')  # set to integer hex format
        q_dict = {'WI':3, 'WF':0, 'ovfl':'wrap', 'quant':'round'}
        self.my_q.set_qdict(q_dict)
        yq_list = list(map(self.my_q.float2frmt, self.y_list))
    ...
    def test_float2frmt_csd(self):
        ...
        # Integer case: Q3.0, scale = 1, scalar parameter
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'csd')  # set to integer csd format
        q_dict = {'WI':3, 'WF':0, 'ovfl':'sat', 'quant':'round'}
        self.my_q.set_qdict(q_dict)
        ...
        fb_set('qfrmt', 'qfrac')
        fb_set('fx_base', 'csd')  # set to fractional csd format
        self.my_q.set_qdict({'WI': 0, 'WF': 6})
        yq_list = list(map(self.my_q.float2frmt, y_list))
...
#================== FRMT -> FLOAT ===============================================
    ...
    def test_frmt2float_float(self):
        ...
        self.my_q.set_qdict(q_dict)
        # scalar format
        yq_list = list(map(self.my_q.frmt2float, self.y_list))
        yq_list_goal = self.y_list
        self.assertEqual(yq_list, yq_list_goal)
        # vector format
        yq_list = list(self.my_q.frmt2float(self.y_list))
        self.assertEqual(yq_list, yq_list_goal)

        # input list of strings, otherwise same - scalar
        y_string = ['-1.1', '-1.0', '-0.5', '0', '0.5', '0.9', '0.99', '1.0', '1.1']
        yq_list = list(map(self.my_q.frmt2float, y_string))
        self.assertEqual(yq_list, yq_list_goal)
        # vector doesn't work yet
        yq_list = list(self.my_q.frmt2float(y_string))
        self.assertEqual(yq_list, yq_list_goal)
    ...
    def test_frmt2float_bin(self):
        ...
        self.assertEqual(yq_list, yq_list_goal)
        # same but vectorized
        #yq_list = list(self.my_q.frmt2float(y_list))
        #self.assertEqual(yq_list, yq_list_goal)
    ...
    def test_frmt2float_hex(self):
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'hex')  # set to integer format
        q_dict = {'WI':3, 'WF':0, 'ovfl':'sat', 'quant':'round'}
        self.my_q.set_qdict(q_dict)
        yq_list = list(map(self.my_q.frmt2float, y_list))
    ...
    def test_frmt2float_csd(self):
        ...
        fb_set('qfrmt', 'qint')
        fb_set('fx_base', 'csd')  # set to integer format
        q_dict = {'WI':4, 'WF':0, 'ovfl':'sat', 'quant':'round'}
        self.my_q.set_qdict(q_dict)
        yq_list = list(map(self.my_q.frmt2float, y_list))
