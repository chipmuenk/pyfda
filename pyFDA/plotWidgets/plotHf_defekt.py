...
#    def __init__(self, parent=None):      
 ...
 #       self.createMPLCanvas()   
        self.MC = plotAll.createMPLCanvas()   
        self.update(coeffs)
...
#        self.connect(self.cboxGrid, SIGNAL('stateChanged(int)'), self.update)
        # self.cb_grid.clicked.connect(self.update) 
...
#        self.connect(self.sldLw, SIGNAL('valueChanged(int)'), self.redraw)
    ...
    def update(self, coeffs = (1,1)):
        ...
        #
        self.MC.axes.clear()        
        self.MC.axes.axis([0, 0.5, -self.A_SB-10, 2])

        self.pltPlt, = self.MC.axes.plot(F,20*np.log10(abs(H)),
                       lw = self.MC.sldLw.value())
        self.MC.axes.set_xlabel(r'$F\; \rightarrow $')                       
        self.MC.axes.set_ylabel(r'$|H(\mathrm{e}^{\mathrm{j} \Omega})|\; \rightarrow $')
        self.MC.axes.set_title(r'Betragsfrequenzgang')
        self.redraw()
#        self.MC.canvas.draw()
    ...
    def redraw(self):
        ...
        """
#        self.MC.axes.grid(self.cboxGrid.isChecked())
        plt.artist.setp(self.pltPlt, linewidth = self.MC.sldLw.value())
        self.MC.fig.tight_layout()
        self.MC.canvas.draw()
